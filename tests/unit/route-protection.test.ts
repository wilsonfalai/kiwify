...
import { middleware as checkoutMiddleware } from "../../apps/checkout/middleware.js";
...
describe("route protection", () => {
  it("blocks anonymous users from private frontend surfaces", () => {
    ...
    expect(checkoutMiddleware({ headers: new Headers() })).toEqual({ allowed: false, redirectTo: "/login" });
  });
  ...
  it("allows authenticated users through private frontend middleware", () => {
    ...
    expect(checkoutMiddleware({ headers: headersWithSession("buyer") })).toEqual({ allowed: true });
  });
});
