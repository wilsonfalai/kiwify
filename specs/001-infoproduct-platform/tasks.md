...
- [ ] T031 Criar tabela `orders` em `packages/database/src/schema/orders.ts`
  - Objetivo: Persistir lifecycle de pedido. Arquivos prováveis: `packages/database/src/schema/orders.ts`. Critério de aceite: status pending/paid/failed/canceled e totais existem. Teste obrigatório: schema assertion. Comando para validar: `pnpm --filter @kiwifyclone/database test:unit -- --run orders-schema`
...
- [ ] T073 [US2] Criar validação mínima de documento em `packages/schemas/src/customer.ts`
  - Objetivo: Validar CPF/CNPJ ou documento mínimo aceito. Arquivos prováveis: `packages/schemas/src/customer.ts`. Critério de aceite: formatos inválidos bloqueiam checkout. Teste obrigatório: schema unit. Comando para validar: `pnpm --filter @kiwifyclone/schemas test:unit -- --run customer-document`
- [ ] T074 [US2] Criar status de pedido em `packages/schemas/src/order.ts`
  - Objetivo: Padronizar `pending`, `paid`, `failed`, `canceled`. Arquivos prováveis: `packages/schemas/src/order.ts`, `packages/database/src/schema/orders.ts`. Critério de aceite: API e DB usam enum compartilhado. Teste obrigatório: enum consistency. Comando para validar: `pnpm --filter @kiwifyclone/schemas test:unit -- --run order-status`
- [ ] T075 [US2] Criar status de pagamento em `packages/schemas/src/payment.ts`
  - Objetivo: Padronizar `pending`, `confirmed`, `failed`, `refunded`. Arquivos prováveis: `packages/schemas/src/payment.ts`, `packages/database/src/schema/payments.ts`. Critério de aceite: mapper Asaas usa enum compartilhado. Teste obrigatório: enum consistency. Comando para validar: `pnpm --filter @kiwifyclone/schemas test:unit -- --run payment-status`
...
- [ ] T077 [US2] Garantir que dados sensíveis de cartão não sejam persistidos em `apps/api/test/payment-security.e2e-spec.ts`
  - Objetivo: Provar ausência de cartão sensível no banco/logs. Arquivos prováveis: `apps/api/test/payment-security.e2e-spec.ts`, `packages/test-utils/src/assert-no-sensitive-card-data.ts`. Critério de aceite: buscas por número/CVV/token falham em tabelas permitidas e logs. Teste obrigatório: payment security integration. Comando para validar: `pnpm --filter @kiwifyclone/api test:integration -- --run payment-security`
...
- [ ] T082 [US4] Garantir idempotência por eventId em `apps/api/src/webhooks/webhook-idempotency.service.ts`
  - Objetivo: Impedir duplicidade no intake. Arquivos prováveis: `apps/api/src/webhooks/webhook-idempotency.service.ts`. Critério de aceite: eventId repetido vira no-op seguro. Teste obrigatório: duplicate intake integration. Comando para validar: `pnpm --filter @kiwifyclone/api test:integration -- --run webhook-idempotency`
